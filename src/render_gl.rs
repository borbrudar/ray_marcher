...
impl Shader {
    pub fn from_file(path: String, kind: gl::types::GLenum) -> Result<Shader, String> {
...
fn preprocess(path: String) -> String {
    // location of the preprocessed file
    ...
    while bad == 0 {
        ...
        let lines: Vec<Result<String, std::io::Error>> =
            BufReader::new(fs::File::open(out.clone()).expect("buffered reader fck up"))
                .lines()
                .collect();
        let _ = fs::remove_file(out.clone());
